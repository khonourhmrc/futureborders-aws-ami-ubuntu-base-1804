...
export AWS_ENVIRONMENT_TAG=packer
...
AWS_SUBNET_ID=$(aws ec2 describe-subnets --filter "Name=vpc-id,Values=${AWS_VPC_ID}" "Name=tag:Name,Values=packer*" --output text --query 'Subnets[0].SubnetId')
