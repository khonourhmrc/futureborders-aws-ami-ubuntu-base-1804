...
node ('master') {
  ...
  stage('Build') {
    ansiColor('xterm') {
      ...
      sh("source env.sh && packer build -var-file=jenkins-packervars.json templates/ami-ubuntu-1804-base.json")
    }
...
def RunTest(String TestType){
  stage(TestType) {
    ansiColor('xterm') {
      ...
      sh("JUNIT_XML=${env.BUILD_TARGET}/${TestType}/${TestType}_test_kitchen.xml TEST_KITCHEN_AMI=\$(cat ${env.BUILD_TARGET}/ami_id.txt) bundle exec kitchen verify ${TestType}")
      if (TestType == 'inspec') {
